...
- Also: `shared/app-brand.css` had drifted out of this session's sync routine — only `shared/app-shell.js` was being kept in sync across the 7 consumer apps, even though `app-brand.css` changes (like this one) need the same treatment. Re-synced `app-brand.css` to all 7 consumers as part of this fix; both shared files should be synced together going forward whenever either changes.

## 1.17.3 — 2026-09-25

- The Updates feature's per-entry Copy button now copies rich formatting (bold title, date on its own line, then the bullet list) instead of flat plain text — matching how this project's own news-item copy already formats a paste into email/docs. Added a new generic `[data-copy-html]` opt-in to the shared copy-to-clipboard handler (falls back to the existing plain-text `[data-copy-value]` copy if the rich copy fails), so any consumer app can opt a copy button into rich formatting the same way.
- Each Updates entry's copied text/HTML (Copy button — not the rendered card) now leads with its app name (from `[data-app-name]`) in bold with a ✨ in front ("✨ **App Name updates**"), a blank line, then the update itself, ending with a bold "👉 **Visit App Name**" link to that app's own site (`https://marincountygov.github.io/<repo>/`) — so an entry copied or shared on its own still says which app it's about, without adding anything to the page itself.
- Fix the Visit-app-site link (and the app-name kicker) not actually reaching the clipboard on a plain-text paste: the rich copy used `execCommand("copy")` against a real DOM selection, and browsers derive that plain-text flavor themselves from the selection's visible text — a link's `href` is dropped, keeping only its label. Copy-to-clipboard buttons with `[data-copy-html]` now use the async Clipboard API (`navigator.clipboard.write` with a `ClipboardItem`) to write the exact `text/plain` and `text/html` content asked for, falling back to the old `execCommand` technique and then plain `writeText` only for browsers without that support.
- Fix an extra blank line between the bullet list and the Visit-app-site link in the copied rich HTML: a `<br>` was added unconditionally before that link, doubling up with `<ul>`'s own block spacing when a bullet list was present. The `<br>` is now only added when there's no bullet list to provide that break itself.
- The copied kicker line now reads "✨ **App Name updates — _Sep 25, 2026_**" (sparkle emoji, "App Name updates" in bold, the date bold *and* italic — nested the same way this project's own news-digest header nests its date inside its bold wrapper) and the copy no longer repeats the per-title date line right under the title, since the kicker already carries the date. The rendered card itself is unchanged — it still shows its own date under the title, as before.

## 1.17.2 — 2026-09-25

- Fix the Updates feature's commit-body rendering: it treated every raw line of a commit body as its own bullet, but this project's own commit convention is hard-wrapped prose paragraphs, not markdown bullet lists — a wrapped sentence was rendering as several unrelated-looking `<li>` fragments, split at arbitrary ~72-character wrap points instead of real sentence boundaries. Every body now renders as a `<ul>` with one `<li>` per actual point: for a genuine bullet list (`-`/`*`-prefixed lines), that's each bullet, with a non-prefixed line right after one correctly merged back in as that bullet's own wrapped continuation rather than treated as a new item; for prose, the wrapped lines are reflowed into one block first, then split into real sentences (a regex that keeps abbreviations, decimals/version numbers, and dotted names like "Node.js" intact, verified directly against this project's own actual commit text).
- Fix the Copy button's copied text to include the date (was title + body only, no date).
- Fix `escapeHtml()`: it didn't escape `"`, but its result gets embedded inside a double-quoted `data-copy-value="..."` attribute — any commit title or body containing a literal `"` (not rare — e.g. a quoted phrase like `"Marin County"`) broke that attribute and corrupted the surrounding markup. This was a pre-existing bug, not introduced by the above; now matches the complete `escapeHtml()` other consumers already use (e.g. marin-mentions' own `assets/app.js`).
